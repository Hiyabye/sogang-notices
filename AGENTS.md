...
`ocr/run_bellarmine.py` consumes only bounded local preparation/images, verifies byte identity before and after extraction, and emits typed ok/rejected results after all date/anchor/geometry/quality checks. `ocr/download_models.py` is an explicit separate installation step; hashes are checked after cache restore and reads/downloads have a 32-MiB asset ceiling. `ocr/extract_bellarmine.py` and `benchmark_bellarmine.py` remain diagnostic tools: their `publicationReady: false` documents are not publication inputs. Ordinary npm tests/notice-only collection never install or invoke Python. Read `ocr/README.md` for exact setup and outstanding image failures.

Keep Python dependencies isolated; generated `ocr/requirements.lock` changes only through its documented resolver command. Models, real source images, diagnostic output and Python caches are not committed or published. Publication run 34839647879 at `69efae3` passed Linux dependency/model installation, 73 Node tests, 34 Python tests and OCR execution, then deployed all 41 notice feeds plus unavailable/date-mismatch meals. Actual batch 6/6 attempted seven boards without recovery failures and carried 34. All 42 live paths validated with JSON UTF-8 and CORS `*`; Firefox consumed real meals from local and deployed Rill. This verifies the rejection path, not accepted real-week quality. The owner confirmed source-use permission; no independent license determination was made. Expanded permitted image fixtures/full human ground truth, an accepted unseen normal week, complete supply-chain/license review and scheduled execution of this revision remain outstanding. Do not release unvalidated full-table output. Downstream code is implemented and tested with synthetic accepted/rejected results; that is not a claim of real-menu production readiness.
